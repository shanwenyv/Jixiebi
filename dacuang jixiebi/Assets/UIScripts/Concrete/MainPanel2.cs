...
/// Main场景主面板
/// </summary>
public class MainPanel2 : BasePanel
{
    bool pauseAndResume = true;
    static readonly string path = "Perfabs/UI/Panel/MainPanel2";
    public MainPanel2() : base(new UIType(path)) { }
    #region
    Button exitButton;
    ...
    public override void OnEnter()
    {
        ...
        {
                PanelManager.Push(new MessagePanel2());
        });
        ...
        UITool.GetOrAddComponentInChildren<Button>("ProgrammeButton").onClick.AddListener(() =>
        {
                ...
                PanelManager.Push(new ProgrammePanel2());
        });
        UITool.GetOrAddComponentInChildren<Button>("InitializeButton").onClick.AddListener(() =>
        {
            IRB4600JointInitiaze.IRB4600Initiaze();
            if (IRBJointControl.Angles)
                IRB4600JointInitiaze.IRB4600InitiazeReturn();
            IRBJointControl.ResetAngles();
        });
        UITool.GetOrAddComponentInChildren<Button>("AutomaticButton").onClick.AddListener(() =>
        {
            IRBJointControl.automatic = true;
        });
        UITool.GetOrAddComponentInChildren<Button>("GuideButton").onClick.AddListener(() =>
        {
            if (IRBJointControl.guide.activeInHierarchy == false)
                IRBJointControl.guide.SetActive(true);
            else
                IRBJointControl.guide.SetActive(false);
        });
    ...
    private void PauseAndResume()
    {
        if (pauseAndResume)
        {
            IRBAngleMessage.GetAngleSpeed();
            IRBAngleMessage.SetSpeedZero();
            UITool.GetOrAddComponentInChildren<Text>("PauseAndResume").text = "恢复运行";
            pauseAndResume = false;
        }
        else
        {
            IRBAngleMessage.SetAngleSpeed();
